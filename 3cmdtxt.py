...
print("------------------------------->>> Welcome To Pied Piper Assitant Wizard <<<-------------------------------------",end='')
print("_________________________________________________________________________________________________________________")
...
while done!=1 :
	if(flag==0):
		print("What can i do for you %s?"%name2)
	else :
	...
	flag=1
	if(p==" "):
		print("I am still waiting for thy command!")
		flag=0
		continue
	
	if(("human" in p) or ("bad" in p) or ("married" in p) or ("robot" in p) or ("old" in p) or ("single" in p)):
		print("Please use the program for its intended purposes.")
		print("The program is still in developmental stage")
		continue
	...
	if(("do not" in p) or ("dont" in p) or ("don't" in p) or ("fake" in p)) and (("run" in p) or ("execute" in p) or ("launch" in p) or ("start" in p) or ("begin" in p) or ("create" in p) or ("initiate" in p) or ("open" in p) or ("activate" in p)):
		print("Ok i will not perform this task.. as you said.")
		continue
	...
	if (("run" in p) or ("execute" in p) or ("launch" in p) or ("start" in p) or ("begin" in p) or ("create" in p) or ("initiate" in p) or ("open" in p) or ("activate" in p)) and (("internet explorer" in p) or ("browser" in p) or ("default web browser" in p) or ("IE" in p) or ("ie" in p)):
		...
		if(x==0):
			print("Oh no. We failed to open this program for you.")
			print("Path for this program is not set in your environment variables")
			if(yt==0):
				...
				print("To set the path in future in your device, please follow this link")
		...
		print("Thank you for using Pied Piper Assistant wizard!")
		...
		print("\nYou seem to be lost?")
		print("Would you like to see the list of available commands?")
		print("Please enter yer or no:",end=' ')
		lost=input()
		if(("yes" in lost) or ("y" in lost) or ("sure" in lost) or ("offcourse" in lost)):
			print("Here is the list of all the currently available commands in this program!")
			print()
		...
		print("Looks like you have entered na invalid command")
